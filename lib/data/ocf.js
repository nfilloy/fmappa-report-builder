import { SCOPE_COLORS, SEQUENTIAL_COLORS } from "../report/chartTheme.js";
import { pickEmissionUnit } from "../formatters.js";
import { buildReportTitle } from "../report/sections.js";

import { parseNumberStrict, percentage, sumBy } from "./numeric.js";

export const OCF_REQUIRED_COLUMNS = [
  "entity",
  "total_emissions",
  "total_scope_1",
  "total_scope_2",
...
// Common labels for the company-wide total row, matched case-insensitively. The
// detection must NOT depend on the Spanish "Total empresa" literal, or any other
// label is mis-read as a plant and double-counted into the headline figure.
...
function normalizeRecord(record, warnings) {
  const entity = record.entity?.trim() || "Unnamed entity";

  // Parse a numeric cell and record a warning when a non-empty value could not
  // be read (it is treated as 0). Empty cells are legitimate zeros, no warning.
  const num = (column) => {
    const { value, malformed } = parseNumberStrict(record[column]);
    if (malformed) {
      warnings.push({ row: entity, column, rawValue: record[column] });
    }
    return value;
  };

  const categories = CATEGORY_COLUMNS.reduce((accumulator, key) => {
    accumulator[key] = num(key);
    return accumulator;
  }, {});

  return {
    entity,
    totalEmissions: num("total_emissions"),
    scope1: num("total_scope_1"),
    scope2: num("total_scope_2"),
    scope3: num("total_scope_3"),
    categories,
    raw: record,
  };
}

// Locate the company-wide total row without depending on its label. Returns its
// index, or -1 when none can be confidently identified.
function findTotalRowIndex(normalizedRows) {
  // 1. Flexible label match against the common total labels.
  const byLabel = normalizedRows.findIndex((row) =>
    TOTAL_ROW_LABELS.has(row.entity.trim().toLowerCase()),
  );
  if (byLabel !== -1) {
    return byLabel;
  }

  // 2. Structural detection: the row whose total emissions ≈ the sum of all the
  //    other rows (≈1% relative tolerance for rounding). Used only if exactly
  //    one row matches, otherwise it is ambiguous.
  if (normalizedRows.length < 2) {
    return -1;
  }

  const grandTotal = sumBy(normalizedRows, (row) => row.totalEmissions);
  let match = -1;
  for (let index = 0; index < normalizedRows.length; index += 1) {
    const value = normalizedRows[index].totalEmissions;
    const others = grandTotal - value;
    if (others <= 0) {
      continue;
    }
    if (Math.abs(value - others) <= others * 0.01) {
      if (match !== -1) {
        return -1;
      }
      match = index;
...
export function validateOcfRows(rows) {
  ...
  const missing = OCF_REQUIRED_COLUMNS.filter((column) => !columns.includes(column));
...
export function buildOcfReport(rows, fileName = "Uploaded CSV") {
  ...
  const warnings = [];
  const normalizedRows = rows.map((record) => normalizeRecord(record, warnings));
  const totalIndex = findTotalRowIndex(normalizedRows);
  const totalRow = totalIndex === -1 ? null : normalizedRows[totalIndex];
  // Exclude the detected total row by index/identity, never by label, so a total
  // row is never rendered as a phantom plant nor summed into the headline.
  const siteRows = normalizedRows.filter((_, index) => index !== totalIndex);
  ...
  return {
    ...
    reportTitle: buildReportTitle("ocf"),
    ...
    unit: pickEmissionUnit(total.totalEmissions, "ocf"),
