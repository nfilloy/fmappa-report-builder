...
// The model is built from the in-boundary stage columns, so they are required:
// a CSV with only product/functional_unit/total_emissions would otherwise yield
// a 0 footprint while total_emissions is non-zero (self-contradictory).
// Downstream stage columns (distribution/use/end_of_life) stay optional → 0.
export const PCF_REQUIRED_COLUMNS = [
  "product",
  "functional_unit",
  "total_emissions",
  "total_materials",
...
//   - cradle-to-gate: only the upstream-to-gate stages count towards the
//     footprint; downstream stages depend on the product's final application.
...
function normalizeRecord(record, includedStageGroups, warnings) {
  const product = record.product?.trim() || "Unnamed product";
  ...
  const num = (column) => {
    ...
    if (malformed) {
      warnings.push({ row: product, column, rawValue: record[column] });
    }
    return value;
  };

  const stageTotals = PCF_STAGE_GROUPS.reduce((accumulator, group) => {
    accumulator[group.valueKey] = num(group.totalColumn);
    return accumulator;
  }, {});
  ...
  return {
    ...
    cradleToGrave: num("total_emissions"),
    inBoundaryTotal,
...
export function validatePcfRows(rows) {
  ...
  const missing = PCF_REQUIRED_COLUMNS.filter((column) => !columns.includes(column));
...
export function buildPcfReport(rows, fileName = "Uploaded CSV", options = {}) {
  ...
  const products = rows.map((record) =>
    normalizeRecord(record, INCLUDED_STAGE_GROUPS, warnings),
  );
  ...
  return {
    ...
    reportTitle: buildReportTitle("pcf"),
    standard: "ISO 14067",
