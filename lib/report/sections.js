...
function pcfContentById(id, report) {
  ...
  // Boundary-aware copy. `cradle-to-gate` keeps the Relats intermediate-component
  // narrative (downstream excluded); `cradle-to-grave` reports the full life
  // cycle without implying Relats sells final consumer products. The "comparative
  // portfolio sum, not additive" caveat is kept in BOTH modes (it depends on
  // differing functional units, not on the boundary).
  ...
  const content = {
    ...
    limitations: isGate
      ...
      : "This configurable report presents the calculated cradle-to-grave footprint from the uploaded dataset; it does not recalculate emission factors or audit the underlying activity data. Because products use different functional units, the combined figure is a comparison rather than an additive total. The full life cycle is reported, so the downstream distribution, use and end-of-life stages depend on assumptions about each product's final application. The products shown here are illustrative sample data; the template adapts to the real Relats product families supplied in the source CSV. Planned enhancements include deeper primary-data collection for the most material lifecycle categories and an externally assured reporting cycle.",
